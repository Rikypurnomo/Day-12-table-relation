...
func editProject(w http.ResponseWriter, r *http.Request) {
	...
	err = config.Conn.QueryRow(context.Background(), "SELECT * FROM tb_projects WHERE id=$1", id).Scan(
		&resultData.ID, &resultData.ProjectName, &resultData.StartDate, &resultData.EndDate, &resultData.Description, &resultData.Technologies, &resultData.UserId, &resultData.Image,
	)
